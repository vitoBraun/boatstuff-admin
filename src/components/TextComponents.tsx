...
export const TextField = memo(({ onChange, value, name, label, ...restProps }: { onChange?: (e: ChangeEvent<HTMLInputElement>) => void, value?: string | number | null, name?: string, label?: string }) => {
...
export const TextArea = memo(({ onChange, value, name, label, ...restProps }: { onChange: (e: ChangeEvent<HTMLTextAreaElement>) => void, value: string | number, name: string, label: string }) => {
    return <><label htmlFor="title" className="text-sm text-navy-700 font-bold">{label}</label>
        <textarea
            rows={5}
            name={name}
